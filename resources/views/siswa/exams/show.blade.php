    ...
    <div class="py-8"><div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8"><div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <div class="mt-6">
            @if ($attempt && $attempt->status !== 'in_progress') 
                <a href="{{ route('siswa.ujian.result', $exam) }}" class="inline-flex rounded-lg bg-blue-600 px-5 py-3 text-sm font-bold text-white shadow-sm hover:bg-blue-700 transition">Lihat Hasil</a>
            @elseif ($exam->status !== 'aktif')
                <p class="text-sm font-semibold text-slate-500">Ujian tidak tersedia. Ujian hanya dapat diakses sesuai jadwal dan status.</p>
            @elseif ($status === 'Sedang Berlangsung' && $attempt && session()->get('siswa.exam_token_verified.' . $exam->id, false)) 
                <a href="{{ route('siswa.ujian.work', $exam) }}" class="inline-flex rounded-lg bg-blue-600 px-5 py-3 text-sm font-bold text-white shadow-sm hover:bg-blue-700 transition">Lanjutkan Ujian</a>
            @elseif ($status === 'Sedang Berlangsung' && $exam->questions_count > 0 && $exam->token_aktif && $exam->token) 
                <a href="{{ route('siswa.ujian.token', $exam) }}" class="inline-flex rounded-lg bg-blue-600 px-5 py-3 text-sm font-bold text-white shadow-sm hover:bg-blue-700 transition">Mulai Ujian</a>
            @else 
                <p class="text-sm font-semibold text-slate-500">{{ $status }}. Ujian hanya dapat dikerjakan sesuai jadwal.</p> 
            @endif
        </div>
    </div></div></div>
</x-app-layout>